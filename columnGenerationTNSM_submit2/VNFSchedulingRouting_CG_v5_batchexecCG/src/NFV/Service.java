...
public class Service implements Serializable {
	 ...
	 */
	public Service copy(boolean copyMapping)
	{
		Middlebox m =null;
		Middlebox b;
		VLink l = null;
		...
		for (int i=0; i<this.middleboxes.size(); i++)
		{
			...
			m = new Middlebox(b.getId(), this, b.getType(), b.getProcessingTime());
		...
		for (int i=0; i<this.virtualLinks.size(); i++)
		{
			...
			l= new VLink(v.getId(), v.getContiniousId(), this, v.getSource(), v.getDestination());
    ...
    {
    	Middlebox m,mp;
    	int [] timeGap = new int[this.middleboxes.size()];
    	Middlebox [] boxes = new Middlebox[this.middleboxes.size()];
    	int temp;
    	
    	//set the flow time of the first middlebox =0;
    	timeGap[0] = 0;
    	//start from 1 to be able to check i-1
    	for(int i=1; i<this.middleboxes.size(); i++)
    	{
    		mp = this.middleboxes.get(i);
    		m = this.middleboxes.get(i-1);
    		timeGap [i]= mp.getStartedProcessing()- m.getIsProcessing()[m.getProcessingTime()-1];    	
    		boxes[i] = mp;
    	}
    	
    	//order middleboxes
    	for (int i=0; i<timeGap.length; i++)
    	{
    		for (int j=i+1; j<timeGap.length; j++)
    		{   		      
                if (timeGap[j]< timeGap[i]) 
                {
