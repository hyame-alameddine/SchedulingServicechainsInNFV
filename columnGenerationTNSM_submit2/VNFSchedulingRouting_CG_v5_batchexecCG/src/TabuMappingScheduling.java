...
public class TabuMappingScheduling {
	 ...
	 */
	public Service scheduleServiceFCFS (Service s, int timeslot, ArrayList<ArrayList<Integer>> vnfs, ArrayList<ArrayList<Integer>> links, boolean updateVnfsLinks)
	{		
		Middlebox m;
		VLink l;
	 ...
	 */
	public Service  mapScheduleService(Service s, int timeslot, ArrayList<ArrayList<Integer>>vnfs,ArrayList<ArrayList<Integer>>links, boolean updateVnfsLinks, boolean map  )
	{
	...
	public Service tabuMappingScheduling (Service s, int iterations, int timeslot, ArrayList<ArrayList<Integer>>vnfs,ArrayList<ArrayList<Integer>>links)
	{
		...
		//no mapping for the service initially
		Service initialServiceSolution = s.copy(false);
		...
		initialServiceSolution= mapScheduleService(initialServiceSolution, timeslot, vnfs, links, updateVnfsLinks, true);
		...
		//initial solution time gap/ time gap of m 
		int z0 = m.getTimeGap();
		...
		//if there is only one vnf of the sam type of m then that would be the one on which it is already mapped, choose the next middlebox with the biggest flow time
		while (neighborhood.size() ==1)
		{
			m =initialServiceSolution.getMiddleboxesTimeGapOrdered()[k];
			k++;
		}
		...
		while (count != iterations )
		{
			for (int i=0; i<neighborhood.size(); i++)
			{
				...
				tempService = initialServiceSolution.copy(true);
				tempService.getMiddleboxes().get(m.getId()).setMappedToVNF(neighborhood.get(i));
				tempService = this.mapScheduleService(tempService, timeslot, vnfs, links, updateVnfsLinks, map);				
