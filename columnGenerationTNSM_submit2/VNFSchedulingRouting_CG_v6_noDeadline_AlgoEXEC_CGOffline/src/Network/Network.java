...
public class Network implements Serializable{
	...
	/**
	 * this method will return a vnf of a specified id
	 * @param id
	 * @return
	 */
	public VNF getVNF(int id)
	{
		for (int i=0; i<this.pmList.length; i++)
		{
			for (int j=0; j<this.pmList[i].getHostedVNFs().size(); j++)
			{
				if (this.pmList[i].getHostedVNFs().get(j).getId() == id)
				{
					return this.pmList[i].getHostedVNFs().get(j);
				}
			}
		}
	...
	public ArrayList<ArrayList<Integer>> getLinksTimeBw(int size)
	{
		...
		for (int i=0; i<this.linkList.length; i++)
		{
			...
			{
				links.get(i).add(this.linkList[i].getCapacity());
			}
		}
