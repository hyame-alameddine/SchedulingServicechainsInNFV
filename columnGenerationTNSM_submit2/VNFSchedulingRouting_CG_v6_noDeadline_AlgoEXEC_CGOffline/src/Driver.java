...
public class Driver {
		...
		public void runColumnGeneration (int setId, int setSize, int totalSets, String servicesSerializedFile, String masterFilePath,String ilpFilePath, String pricingFilePath, int sheetNb) throws IloException, IOException, RowsExceededException, BiffException, WriteException
		{
			...
			//	testResults = CG.executeColumnGeneration(false, masterFilePath, ilpFilePath, pricingFilePath, false);
				testResults = CG.executeColumnGenerationWithThread(false, masterFilePath, ilpFilePath, pricingFilePath, false);
		...
		public void runColumnGenerationWithDiversification (int setId, int setSize, int totalSets, String servicesSerializedFile, String masterFilePath,String ilpFilePath, String pricingFilePath, int sheetNb, boolean useIncumbentCallback, boolean useHeuristicDiversification, int startTimeslot,int nbOfConfigPerService) throws IloException, IOException, RowsExceededException, BiffException, WriteException
		{
			...
			for (int i=0; i<totalSets; i++)
			{
				...
				testResults = CG.executeColumnGenerationDiversificationWithThread(useIncumbentCallback, useHeuristicDiversification, startTimeslot, nbOfConfigPerService, masterFilePath, ilpFilePath, pricingFilePath);
				//write test results to the excel sheet
		...
		public static void resultsRunIlpComparison() throws Exception
		{
			//network input
			...
			int [] servicesSize = {6,9,15,60,90,180};//{3,6,9,12};
			int totalSets = 1;
			int sheetNb =0;
			int setId =0;
			//files
			...
			String resultstxt = "testResults/CGDiverresultstxt.txt";
			...
			SchedulingHeuristic sched = new SchedulingHeuristic(n,"logs/ServicesHeuristic.txt");
			...
			for(int i=0; i<servicesSize.length; i++)
			{
				...
				drive.runColumnGenerationWithDiversification(setId, 6, totalSets, servicesSerializedFile, divMasterFilePath, divIlpFilePath, divPricingFilePath, sheetNb, false, true, 0, servicesSize[i]);
				//drive.runILPModel(setId, servicesSize[i], totalSets, servicesSerializedFile, ILPModelFilePath, sheetNb);
				
				//only tabu +sequential ilp
				/*ServicesDriver driver = new ServicesDriver (servicesSize[i],  delta, minVm, maxVM, minBw, maxBw,maxProcessing , minProcessing, minTraffic, maxTraffic,n.getPhysicalNetworkArray().get(2)[0]);
				ArrayList<Service> services = driver.generateServices(arrivalRate, departureRate);
				...
				double start = System.currentTimeMillis();
				 int sc = sh.tabuMappingScheduling(services,4, 0, vnfs, links,100);//try 5
				double end = System.currentTimeMillis();
				drive.resultsFile.writeInFile(sc+"\t");
				drive.resultsFile.writeInFile(end-start+"\t");
				//end tabu
				
				//start sequential
				
				double start = System.currentTimeMillis();
				double sc = sched.mapScheduleServiceILPSequentially(services, 0, 100);
				double end = System.currentTimeMillis();
				drive.resultsFile.writeInFile(sc+"\t");
				drive.resultsFile.writeInFile(end-start+"\t");*/
				//end sequential
