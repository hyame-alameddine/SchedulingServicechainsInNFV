...
public class OnlineScheduling {
	...
	public int periodicOnlineAlgo (int adjustmentValue, String algorithm, int tabuIterations) throws Exception
	{
		...
		//start with the first time period
		int startTimeslot =30;//0;
		int endTimeslot = 40;// this.timePeriod;
		Batch b;
		int processedServices =16;//0;
		ArrayList<Service> servicesBatch;
		...
		while (processedServices<this.services.size())
		{
			...
			if(algorithm.equals("TABU"))
			{
				...
				batchScheduleLength = sh.tabuMappingScheduling(b.services,tabuIterations, b.endTimeslot, vnfs, links,adjustmentValue);//start the batch run after collecting its services
	...
	*  @param services services needed only when we have an optimal solution not got from a batch of a previous run but from serialized file.
	...
	public Batch onlineCGWithThread(Batch batch, boolean diversify, int nbOfConfigPerService, String masterFilePath,String ilpFilePath, String pricingFilePath) throws IloException, IOException
	{
		...
		TabuMappingScheduling tab = new TabuMappingScheduling(this.network, "logs/tabuHeuristic.txt");
		int delta = tab.tabuMappingScheduling(batch.services,4, feasiblSchedStart, vnfs, links,100);
		 for(int i=0; i<batch.services.size(); i++)
		 {
			 try {
				 Configuration t = sh.buildServiceConfiguration(batch.services.get(i), delta, 0);
				 if (t==null)
				 {
					 System.out.println("config null for services"+batch.services.get(i).getId());
					 return null;
				 }
	...
	{	
		...
		for (int j=0; j<setNb; j++)
		{
			//ArrayList<Service> services =  Driver.deserializeServices("testResults/onlineServices_set"+j+"_seed"+startSeed+".ser", 0, servicesNb);//hh
			n= networkListLinks.get(0);
			int[] tf = n.getPhysicalNetworkArray().get(2)[0];
			ServicesDriver driver = new ServicesDriver (25,  delta, 3, 5, 300, 500, 5 , 1, 500, 1500,tf);
	...
	/*	int DELTA = 50;
		...
		for (int i=0; i<batchSizes.length; i++)
		{
			...
			for (int j=0; j<set; j++)
			{
				//get batch
				b = batches.get(j);
				b.id =0; // reset batch id to 0 since we are testing one batch at a time. Prevent an index out of bound exception in getScheduledAndToScheduleServices()
				newon = new OnlineScheduling(b.services, n, timePeriod, delta);
		...
		System.out.println(totalScheduleLength);
		return totalScheduleLength;
	}
	...
	//	OnlineScheduling.onlineBtachExec(true,3,true);
		...
		//OnlineScheduling.onlineAlgoBatchExec("SequentialILP",4);
