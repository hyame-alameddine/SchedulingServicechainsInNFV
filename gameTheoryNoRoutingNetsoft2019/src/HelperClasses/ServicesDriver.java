...
public class ServicesDriver {
	...
	/**IMPORTANT:
	 * This function has been modified so it does not generate services having the same function type repeated 
	 * Make sure when generated to have the number of VNF types >then the number of functions in the same service 
	 * 
	 ...
	 * @throws Exception 
	 */
	public ArrayList<int[][]> generateServicesForModel (int seed) throws Exception
	{
		Random rand = new Random();
		rand.setSeed(seed);//21//12//234//33
		...
		for (int i=0; i<this.serviceNb; i++)
		{
			...
			for (int j =0; j<this.maxMiddleboxNb; j++)
			{
				...
				else if (j<n-1)
				{
					...
					{
						//IMPORTANT prevent choosing the same vnf type twice for the same service
						do{
						...
						while (Search.exists(this.VNFTypes[c], msn [i]));
			...
			//set the total processing to the middlebox processing time +number of virtual links*transmission delays on each
			totalps[i]=totalps[i]+(n-1)*(ws[0][i]/bs[0][i]);
			
			//Multiplty the processing time by a random number to have a valid deadline (feasible with waiting time)
			
			deadline = totalps[i]+((int)totalps[i]/3);// (int)totalps[i]/3, /2, /1
	
			//choose deadline time to be between total processing time*rand value of all the middleboxes of the service and DELTA
			/*while (deadline>this.totalTimeSlots)
			{
				deadline = totalps[i]*(2+rand.nextInt(4));//totalps[i]*(2+rand.nextInt(6));--used for offline and online lately
				counter++;
				if (counter>100)
				{
					throw new Exception("Error-Can not set Deadline for service "+i+" (processing time*rand value greater that DELTA)");
				}
			}*/
			if (deadline>this.totalTimeSlots)
			{
				throw new Exception("Error-Can not set Deadline for service "+i+" (processing time*rand value greater that DELTA)");
			}
			us[0][i] = deadline;
			deadline=0;
			counter =0;
			//us[0][i] = rand.nextInt((this.totalTimeSlots - (int)(totalps[i]*2)) + 1) + (int)(totalps[i]*2);//update *1.5 for online batch scheduling
	 ...
	 * @param adjustmentValue adjustmanet value for arrival and departue
	 * @param seed seed to generate services
	 ...
	 * @throws Exception 
	 */
	public ArrayList<Service> generateServices (double lambda, double mu, int adjustmentValue, int seed) throws Exception
	{
		//generate the service
		ArrayList<int[][]>  services = this.generateServicesForModel(seed);
		...
		// update deadline to take arrival and departure time into consideration, so if we increase the load we don't fall into the case where service arrive after the deadline
		this.updateServicesDeadlinePoisson (servicesObjects, adjustmentValue, seed);
	...
	/**
	 * 
	 * This method will update the deadline of a service based on its arrival time +(total processing+transmission)*2 of all function and Delta
	 * Used for poisson arrival, it will update arrival and departure time based on the adjustment values
	 * the departure time will be = deadline+departure
	 * @param services
	 * @param adjustmentValue
	 * @return updated services
	 * @throws Exception 
	 */
	public ArrayList<Service> updateServicesDeadlinePoisson (ArrayList<Service> services, int adjustmentValue, int seed ) throws Exception
	{
		Service s;
		int totalps =0;
		Random rand = new Random();
		...
		for(int i=0; i<services.size(); i++)
		{
			...
			{
				totalps+=s.getMiddleboxes().get(j).getProcessingTime();
			}
			
			totalps=totalps+s.getVirtualLinks().size()*(s.getTrafficSize()/s.getBandwdith());
			s.setArrivalTime(s.getArrivalTime()*adjustmentValue);
			
			//min = (int)(totalps*(2+rand.nextInt(4))+s.getArrivalTime());//Multiplty the processing time by a random number to have a valid deadline (feasible with waiting time)
			min =(int)( totalps+((int)totalps/3)+s.getArrivalTime());
			//choose deadline time to be between arrivalTime+total processing time of all the middleboxes of the service and DELTA		
			/*while (min>this.totalTimeSlots)
			{
				min =(int)( totalps+((int)totalps/5)+s.getArrivalTime());
				
				count++;
				if (count>100)
				{
					throw new Exception("Error-Can not set Deadline for service "+s.getId()+" (processing time+arrival greater that DELTA)");
				}
			}*/
			if(min>this.totalTimeSlots)
			{
				throw new Exception("Error-Can not set Deadline for service "+s.getId()+" (processing time+arrival greater that DELTA)");
			}
			
			count=0;
			//s.setDeadlineTime(rand.nextInt((this.totalTimeSlots - min + 1)) + min);// this causes the deadline to change with the change of the load and result on admission increasing with the load
			s.setDeadlineTime(min);
			s.setDepartureTime(s.getDeadlineTime()+s.getDepartureTime()*adjustmentValue);
			totalps =0;
			count=0;
			min=0;
		}
	 ...
	 */
	public static int getHighestDeadline (ArrayList<Service>services)
	{
	...
	//	ArrayList<Service> serviceList = driver.generateServices(20, 10,100);
		ArrayList<Service> serviceList;
		try {
			serviceList = driver.generateServices(20, 10,10,12);
			//System.out.println(serviceList.size());
			for (int i=0; i<serviceList.size(); i++)
			{
				//System.out.println(serviceList.get(i).getArrivalTime()+" < "+serviceList.get(i).getDeadlineTime()+" < "+serviceList.get(i).getDepartureTime());
			}
		} catch (IOException e) {
