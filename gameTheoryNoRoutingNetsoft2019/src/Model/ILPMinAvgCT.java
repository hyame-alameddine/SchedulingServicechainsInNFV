 ...
 * which minimizes the average completion time of the services
 * The objective the model is to minimize the sum completion time of all services which pushes the completion time of each
 */
package Model;
 ...
 * *************************************************************************************************************************/
	...
	//a specifies the completion time of a service
	public IloIntVar[] c;
	 ...
	 * @throws IloException
	 */
    public ILPMinAvgCT(int[][]G, int [][]xfk,  int[]tf, int[]N, int []bs, int []ws, int [] us, int[][] psn,int[][]msn, int delta, int maxMiddlebox) throws IloException
    {
    ...
    public ILPMinAvgCT(Network network, ArrayList<Service> services, int delta) throws IloException
	{ 	
    ...
    public void initializeDecisionVariables() throws IloException
    {
    	...
    	for(int i =0; i<S; i++)
 	    {
 	    	c[i] = cplex.intVar(0, Integer.MAX_VALUE,"c "+i);			
 	    }
    ...
    { 
	    ...
	    for(int i =0; i<S; i++)
	    {
	    	obj = cplex.sum(obj, c[i]);
	    }
	    obj = cplex.prod(1/S,obj);
	...
	/*for (int f=0; f<F; f++)
    {
    	...
    	for (int delta=0; delta<DELTA; delta++)
	    {
  		 ...
  		 * c[i] = sum over f sum over delta y[s][|Ns|][f][delta](delta+psn[s][|Ns|]  for all s, 
  		 */
	    //loop over services
	    for(int i =0; i<S; i++)
	    {
	    	...
	    	cplex.addEq(c[i], processedBeforeDeadlineConstraint, "Const10_S ["+i+"]");
	    }
    ...
    public void runILPModel(String resultsFile) throws IloException
    {
    	...
    	try {
			this.cplex.exportModel("ILPMinSumCT_InputResults.lp");
			if (cplex.solve()) {
    ...
    public void reportResults(String filePath){
		...
		try {
			...
			st+="\t\t"+Output.printArray(c,"c[s]", "\t\tc",this.cplex);
    ...
    public String  printCompletionTimes() throws UnknownObjectException, IloException
    {
    	String st ="";
 	    ...
 	    for(int i =0; i<S; i++)
 	    {
 	    	st+=String.format(cplex.getValue(c[i])+"\t");  
    ...
    public double  getNbOfAdmittedServices() throws UnknownObjectException, IloException
    {
    	int nbOfadmittedServices =0;
 	    ...
 	    for(int i =0; i<S; i++)
 	    {
 	    	 if (cplex.getValue(c[i])<=this.us[i])
			 {
	...
	public static double[] runModelForResults (Network network, ArrayList<Service> services, String resultsFile ) throws IloException
	{
		...
		int delta = ServicesDriver.getHighestDeadline(services)*3;//*2
		System.out.println(delta+"dddddddddd");
		ILPMinAvgCT model = new ILPMinAvgCT(network, services, delta);
	...
	public static void main(String[] args) throws Exception 
	{
		...
		res = ILPMinAvgCT.runModelForResults (network, services,"ILPAvgRes.txt");
