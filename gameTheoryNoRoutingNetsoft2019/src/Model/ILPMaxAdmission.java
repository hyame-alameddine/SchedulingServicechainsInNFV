 ...
 * This is the main ILP of the paper. However comparing maximizing the admission with the game is not very relevant as the game minimize the completion time of each service
 * The objective the model is to maximize the number of admitted services 
 */
package Model;
 ...
 * *************************************************************************************************************************/
	 ...
	 * @throws IloException
	 */
    public ILPMaxAdmission(int[][]G, int [][]xfk,  int[]tf, int[]N, int []bs, int []ws, int [] us, int[][] psn,int[][]msn, int delta, int maxMiddlebox) throws IloException
    {
    ...
    public ILPMaxAdmission(Network network, ArrayList<Service> services, int delta) throws IloException
	{ 	
    ...
    { 
	    ...
	    for(int i =0; i<S; i++)
	    {
	    	...
	    	for (int j=0; j<N[i]; j++)
	    	{
	    		...
	    		cplex.addEq(middleboxMappedConstraint, a[i], "Const2_sn ["+i+"]["+j+"]");//a[i]
	    	}	    			
	    }
	   ...
	   for(int i =0; i<S; i++)
		 ...
		 * sum over f y[s][n+1][f][delta'] <= 1-(sum over f y[n][s][f][delta](1-sum over k h[s][n][k]) for all n,n+1, for all s, for all delta, delta'; delta+psn[s][n]<=delta'<delta+psn[s][n]+d_s
		 */
	    //loop over services
	  for(int i =0; i<S; i++)
	    {
	...
	public static double[] runModelForResults (Network network, ArrayList<Service> services, String resultsFile ) throws IloException
	{
		...
		ILPMaxAdmission model = new ILPMaxAdmission(network, services, delta);
	...
	public static void main(String[] args) throws Exception 
	{
		...
		ServicesDriver driver = new ServicesDriver (7,  300, 3,5, 300,500, 5 , 1, 500, 1500,tf);
		 ArrayList<Service>services = driver.generateServices(20.0,10.0, 100, 14);
		 
		res = ILPMaxAdmission.runModelForResults (network, services,"ILPRes.txt");
		
		int objective = (int)res[0];
		double execTime = res[1];
		
		
		System.out.println ("admittedServicesNb ="+objective+"\t exec Time ="+execTime);
