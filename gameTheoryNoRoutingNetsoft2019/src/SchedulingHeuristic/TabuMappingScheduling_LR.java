/**
 * This tabu search is the same as the one done in Design and evaluation of algorithms for mapping and scheduling of virtual network functions 
 * Here we did not add link scheduling or routing. We account only for transmission delays once pocessing is done
 * This is a modified version (tabu method return admitted seervices not schedule length) of the TabuMappingScheduling we had in CG
 */
...
public class TabuMappingScheduling_LR {
	...
	public TabuMappingScheduling_LR (Network network, String logFileName) throws IOException
	{
	...
	{		
		...
		for (int i=0; i<s.getMiddleboxes().size(); i++)
		{
			...
			if (i < s.getVirtualLinks().size())
			{
				...
				//@added when removing link scheduling if the middleboxes which are source and destination of the links are hosted on the same server no need to route the Vlink
				if (l.areHostedOnSameServer()){
					continue;
				}
	...
	/**
	 * This method performs the tabu mapping, routing and scheduling and returns a list of admitted services (meeet their deadlines)
	 * @param services
	 * @param iterations
	 ...
	 */
	public ArrayList<Service> tabuMappingScheduling(ArrayList<Service> services, int iterations, int timeslot)
	{
		 int highestDeadline = ServicesDriver.getHighestDeadline( services)*4;
		...
		for (int i=0; i<services.size(); i++)
		{
			...
			if (timeslot == -1)
			{
				startTime = (int)(s.getArrivalTime());
			}
			else
			{
				startTime = timeslot;
			...
			if (s!=null && s.getCompletionTime()<=s.getDeadlineTime())
			{
	...
	public static void main(String[]args) throws Exception
	{
		...
		ArrayList<Service> servicesObjects = driver.generateServices(20, 10,100,14);
		
		//schedule the services
		TabuMappingScheduling_LR sh = new TabuMappingScheduling_LR(network,"tabuHeuristic.txt");
		
		//testing for one service
		System.out.println("===Testing mapping algo for one service===");
		ArrayList<Service>admission =	sh.tabuMappingScheduling(servicesObjects,4, 0);//try 5
		System.out.println (admission.size());
